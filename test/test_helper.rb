...
class Nanoc::Blogging::FilterTest < Minitest::Test
  ...
  def mock_article(custom_attrs={})
    ...
    def item.path ; '/meow.html' ; end
    def item.compiled_content(x=nil) ; 'stuff' ; end
    item
  end
  ...
  def mock_feed_item(custom_attrs={})
    ...
    def @item.path ; "/journal/feed/" ; end
    @item
  end
  ...
  def test_atom_feed
    if_have 'builder' do
      ...
      i = @items[1] ; def i.path ; "/item1/" ; end
      i = @items[1] ; def i.compiled_content(snapshot=nil) ; 'item 1 content' ; end
      i = @items[2] ; def i.compiled_content(snapshot=nil) ; 'item 2 content' ; end
